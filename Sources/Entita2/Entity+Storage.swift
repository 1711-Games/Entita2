public extension Entita2Entity {
    ...
    static func loadBy(IDBytes: Bytes, within transaction: (any Entita2Transaction)? = nil) async throws -> Self? {
        try await Self.loadByRaw(
            IDBytes: Self.IDBytesAsKey(bytes: IDBytes)
        )
    }

    static func loadByRaw(IDBytes: Bytes, within transaction: (any Entita2Transaction)? = nil) async throws -> Self? {
    ...
    static func load(by ID: Identifier, within transaction: (any Entita2Transaction)? = nil) async throws -> Self? {
        try await Self.loadByRaw(IDBytes: Self.IDAsKey(ID: ID), within: transaction)
    }

    static func afterLoadRoutines0(maybeBytes: Bytes?, within transaction: (any Entita2Transaction)?) async throws -> Self? {
    ...
    func save0(by ID: Identifier? = nil, within transaction: (any Entita2Transaction)?) async throws {
        let IDBytes: Bytes
        if let ID = ID {
    ...
    func commit0(transaction: (any Entita2Transaction)?) async throws {
        try await transaction?.commit()
    }

    internal func commit0IfNecessary(commit: Bool, transaction: (any Entita2Transaction)?) async throws {
        if commit  {
    ...
    @usableFromInline
    internal static func unwrapTransactionOrBegin(_ transaction: (any Entita2Transaction)?) async throws -> any Entita2Transaction {
        if let transaction {
            return transaction
        }
    ...
    func insert(within transaction: (any Entita2Transaction)? = nil, commit: Bool = true) async throws {
    ...
    func save(within transaction: (any Entita2Transaction)? = nil, commit: Bool = true) async throws {
        try await self.save(by: nil, within: transaction, commit: commit)
    }

    /// Saves current entity to DB within given transaction
    func save(by ID: Identifier? = nil, within transaction: (any Entita2Transaction)? = nil, commit: Bool = true) async throws {
        let transaction = try await Self.unwrapTransactionOrBegin(transaction)
    ...
    func delete(within transaction: (any Entita2Transaction)? = nil, commit: Bool = true) async throws {
        let transaction = try await Self.unwrapTransactionOrBegin(transaction)
